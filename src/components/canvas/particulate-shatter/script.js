...
function updatePaletteUI() {
  ...
  // Create palette colors safely using DOM API
  for (let i = 0; i < PAL_SIZE; i++) {
    const c = palette[i];
    const palColor = document.createElement('div');
    palColor.className = 'pal-color';
    palColor.setAttribute('data-i', i.toString());

    const swatch = document.createElement('div');
    swatch.className = 'pal-swatch';
    swatch.style.background = c ? c.hex : '#cccccc';
    palColor.appendChild(swatch);

    const hexSpan = document.createElement('span');
    hexSpan.className = 'pal-hex';
    hexSpan.textContent = c ? c.hex.toUpperCase() : '—';
    palColor.appendChild(hexSpan);

    if (c) {
      const pctSpan = document.createElement('span');
      pctSpan.className = 'pal-pct';
      pctSpan.textContent = `${c.pct}%`;
      palColor.appendChild(pctSpan);
    }

    // Add click handler safely
    if (c) {
      palColor.addEventListener('click', () => copyColor(i));
    }

    dock.appendChild(palColor);
  }
  ...
  btnCSS.className = 'dock-btn';
  btnCSS.setAttribute('title', 'Copy CSS Variables');
  btnCSS.textContent = '{ }';
  btnCSS.addEventListener('click', copyCSS);
  actions.appendChild(btnCSS);

  const btnSVG = document.createElement('button');
  btnSVG.className = 'dock-btn';
  btnSVG.setAttribute('title', 'Copy SVG');
  btnSVG.textContent = '◇';
  btnSVG.addEventListener('click', copySVG);
  ...
  // Animate in
  const palColors = dock.querySelectorAll('.pal-color');
  palColors.forEach((el, i) => {
    el.style.opacity = '0';
