...
// Coletar todos os PDFs disponíveis na pasta certificados
const certificadosDir = path.join(PUBLIC_DIR, 'certificados');
const pdfsDisponiveis = new Set(
    fs.readdirSync(certificadosDir).map(f => f.toLowerCase())
);

console.log('📋 VERIFICAÇÃO DE PDFs DOS CURSOS\n');
...
cursosData.pt.forEach((curso, index) => {
    totalCursos++;
    const id = curso.id;
...
console.log('\n🇺🇸 CURSOS EM INGLÊS (en):\n');
cursosData.en.forEach((curso, index) => {
    totalCursos++;
    const id = curso.id;
    const titulo = curso.title;
...
console.log(`✅ Cursos com PDF correto: ${cursosComPdfCorreto}`);
...
if (resultados.faltando.length > 0) {
    console.log('\n💡 SUGESTÕES DE CORREÇÃO:\n');
    console.log('Os seguintes cursos estão com caminhos de PDF que não existem:\n');
...
if (resultados.semCampo.length > 0) {
    ...
    resultados.semCampo.forEach(item => {
        console.log(`  [ID ${item.id}] ${item.titulo} (${item.idioma})`);
        console.log(`     ⚠️ O campo 'link' não está preenchido. Adicione o caminho do PDF.\n`);
    });
}
...
if (cursosComPdfFaltando > 0 || cursosSemCampoPdf > 0) {
    ...
    console.log('✅ Todos os cursos estão com PDFs corretos!\n');
    process.exit(0);
}
