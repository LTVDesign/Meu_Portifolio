...
export const useDynamicTextColor = (
  ...
  overrideBgHex?: string
): UseDynamicTextColorResult => {
  const { config } = useParticleConfig();
  const { defaultColorMode } = useDynamicTextContext();

  // Se colorMode não foi explicitamente definido (usou default), usa o do contexto
  const effectiveColorMode =
    colorMode === 'auto' && defaultColorMode !== 'auto' ? defaultColorMode : colorMode;
  ...
  const currentBgHex = useMemo((): string => {
    // Se foi fornecido um override, usa ele (para casos como DynamicText com bg local)
    if (overrideBgHex) {
      return overrideBgHex;
    }
  ...
  const result = useMemo((): UseDynamicTextColorResult => {
    ...
    if (effectiveColorMode === 'dark') {
      ...
      // modo 'auto' - usa contraste ótimo (preto ou branco) baseado na luminância
      textHex = getOptimalTextColor(bgLuminance);
    }
  ...
  }, [currentBgHex, effectiveColorMode, overrideBgHex]);
  ...
  useEffect(() => {
    if (Date.now() - lastLog.current > 1000) {
      ...
      console.log(
        `[DynamicTextColor] Background: ${config.backgroundType} | Cor: ${result.backgroundColor} | Texto: ${result.color} | Luminância: ${result.luminance.toFixed(3)} | Escuro: ${result.isDark}`
      );
    }
