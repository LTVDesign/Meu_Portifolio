...
const GithubIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const LinkedinIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const InstagramIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const FacebookIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const EnvelopeIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const ArrowUpIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const ArrowDownIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
...
const ArrowRightIcon: React.FC<{ className?: string; size?: number }> = ({
  className,
  size = 24,
}) => (
  <svg
      ...
      <div className='footer-content max-w-7xl mx-auto px-[clamp(1rem,4vw,4rem)] relative z-10'>
        ...
        <div className='footer-grid grid grid-cols-[repeat(auto-fit,minmax(min(100%,250px),1fr))] gap-[clamp(1.5rem,4vw,2rem)] justify-items-center items-start text-center mb-[clamp(1.5rem,4vw,2rem)]'>
          ...
          <div className='flex flex-col items-center text-center max-w-sm'>
            <AvailabilityBadge
              availabilityLabel={t('footer.availability', 'Disponível')}
            />
            <h2
              className='text-[clamp(1.2rem,4vw,1.5rem)] font-bold text-white mb-4 uppercase tracking-tighter'
              style={{ textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}
            >
              {t('footer.title', config.html.fullName)}
            </h2>
            <p
              className='text-white/90 text-[clamp(0.85rem,2.5vw,1rem)] leading-relaxed mb-6'
              style={{ textShadow: '0 2px 8px rgba(0,0,0,0.8)' }}
            >
          ...
          <div className='flex flex-col items-center text-center max-w-sm'>
            <h3
              className='text-[clamp(0.7rem,2vw,0.85rem)] font-bold uppercase tracking-widest text-white mb-6'
              style={{ textShadow: '0 0 10px rgba(145, 94, 255, 0.6)' }}
            >
            ...
            <div className='social-links-container flex flex-nowrap items-center justify-center gap-[clamp(0.5rem,1.5vw,1rem)] mb-[clamp(1.5rem,3vw,2rem)]'>
              {socialLinksRaw.map(({ icon: Icon, url, translationKey, color }) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`p-[clamp(0.5rem,1vw,0.75rem)] rounded-xl bg-white/[0.03] border border-white/5 group-hover:border-white/20 group-hover:bg-white/[0.06] ${color} transition-all`}
                    >
                      <Icon size={20} />
                    </div>
                ...
                {showUnlockAnimation && (
                  ...
                  >
                    ...
                    >
                      ...
                      <p className='text-white/60 mt-2'>{t('footer.iniciandoDoom')}</p>
                    </m.div>
                  </m.div>
                )}
              </AnimatePresence>
        ...
        <div className='footer-copyright mt-4 pt-3 border-t border-white/5 flex flex-col items-center gap-1'>
          <p
            className='text-[clamp(0.625rem,1.5vw,0.75rem)] text-white/60 text-center tracking-wide'
            style={{ textShadow: '0 1px 3px rgba(0, 0, 0, 0.5)' }}
          >
            © {new Date().getFullYear()} Leandro Saturnino Barbosa.{' '}
